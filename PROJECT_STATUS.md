...
The production chassis is frozen at the universal standard documented in [CANONICAL_STANDARD.md](CANONICAL_STANDARD.md). The current production candidate is `output/canonical_chassis/`: four boards, each split into four printable sections.

The sloped 2×1 lap-pin assembly in `output/sloped_lap_prototype/` is an experiment only. It must **not** be copied into the sixteen production quadrants until it passes the physical PLA test in [NEXT_PHYSICAL_TEST.md](NEXT_PHYSICAL_TEST.md).
...
The 16 finished production quadrant STLs remain the files in `output/canonical_chassis/{Akala,Melemele,Poni,Ulaula}/quadrant_*.stl`. The largest is Akala Q4 at 229.243 × 229.280 × 10.000 mm, within the 256 × 256 mm A1 build plate and the 248 mm design target.
