...
def build_plan(spacing: float) -> list[tuple[str, list[Placement]]]:
    ...
    prototype = ROOT / "output" / "CURRENT" / "prototype" / "sloped_lap_2x1"
...
def write_report(plan: list[tuple[str, list[Placement]]], spacing: float, destination: Path, preview_dir: Path) -> None:
    ...
    for number, (title, placements) in enumerate(plan, 1):
        ...
        lines += [f"## Plate {number:02d}: {title}", "",
                  ...
                  f"![Plate {number:02d}](../output/CURRENT/a1_print_plan/plate_{number:02d}.png)", "",
...
def main() -> None:
    ...
    parser.add_argument("--output", type=Path, default=ROOT / "output" / "CURRENT" / "a1_print_plan")
    parser.add_argument("--report", type=Path, default=ROOT / "docs" / "A1_PRINT_PLAN.md")
