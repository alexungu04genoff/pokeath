...
from src.prototypes import build_sloped_lap_prototype as sloped
from src.chassis.build_canonical_chassis import export, removal


ROOT = Path(__file__).resolve().parents[2]
DEFAULT_OUT = ROOT / "output" / "CURRENT" / "blender"
PRODUCTION = ROOT / "output" / "CURRENT" / "boards"
SOURCE = ROOT / "output" / "CURRENT" / "prototype" / "sloped_lap_2x1"
...
def write_pair(name: str, mesh: trimesh.Trimesh, output: Path, inventory: list[dict[str, object]], role: str,
    ...
    group = ("tiles" if name.startswith("blank_test_tile") or name == "generic_canonical_36mm_tile"
             else "pins" if name.startswith("pin_")
             else "experimental_parts" if name.startswith(("sloped_part", "sloped_lug", "generic_"))
             else "")
    destination = output / group if group else output
    destination.mkdir(parents=True, exist_ok=True)
    stl = destination / f"{name}.stl"
    obj = destination / f"{name}.obj"
...
def write_readme(output: Path, inventory: list[dict[str, object]]) -> None:
    ...
    rows += ["", "`START_HERE` is the only deliberately assembled multi-part reference mesh. All other files remain separate for editing.",
...
def main() -> None:
    ...
    write_pair("START_HERE", assembled, output, inventory,
