...
OUT=ROOT/'output'/'sloped_lap_prototype'
...
def main():
    ...
    write_readme(report)
    from build_one_plate_test import main as build_plate
    build_plate(OUT)
    import render_a1_placement as placement
    placement.main(OUT)
    for preview in (ROOT/'output/A1_placement_preview').iterdir():
        if preview.is_file(): shutil.copyfile(preview,OUT/preview.name)
    with zipfile.ZipFile(ROOT/'output/sloped_lap_prototype_print_kit.zip','w',zipfile.ZIP_DEFLATED) as archive:
